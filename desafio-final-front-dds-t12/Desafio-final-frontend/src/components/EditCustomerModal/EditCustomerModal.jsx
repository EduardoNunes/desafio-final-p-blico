...
export default function EditCustomerModal({ notify, notifyError }) {
  const {
  ...
  const { setClientCharges } = useChargesContext();

  const [form, setForm] = useState({
    name: clientDataDetails.client.nome,
    email: clientDataDetails.client.email,
    cpf: clientDataDetails.client.cpf,
    phone: clientDataDetails.client.telefone,
    cep: clientDataDetails.client.cep || "",
    address: clientDataDetails.client.endereco || "",
    complement: clientDataDetails.client.complemento || "",
    neighborhood: clientDataDetails.client.bairro || "",
    city: clientDataDetails.client.cidade || "",
    uf: clientDataDetails.client.uf || "",
  });
  ...
  const [existsCpf, setExistsCpf] = useState(false);
  ...
  async function handleSubmitChanges(event) {
    ...
    const { status, ...clientInfos } = clientDataDetails.client;

    let change = false;

    for (var prop in clientInfos) {
      if (clientInfos[prop] !== body[prop]) {
        change = true;
      }
    }

    if (change === false) {
      handleOpenEditCustomerModal(false);
      return;
    }
    ...
    try {
      ...
      const response = await api.put(
        `/updateClient/${clientDataDetails.client.id}`,
        {
          ...body,
        },
        {
      ...
      try {
        const response = await api.get(
          `/clientDetails/${clientDataDetails.client.id}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        const client = {
          ...
          id: clientDataDetails.client.id,
        };
        ...
        <form id="form-modal">
          <div className="form-header">
            ...
            <img
              ...
              src={CloseButton}
              alt="fechar modal"
              onClick={() => handleOpenEditCustomerModal(false)}
            />
          </div>
          ...
          >
            ...
            <FormHelperText sx={{ m: 0, color: "red" }}>
              ...
              {erro && existsEmail && "E-mail já cadastrado"}
            </FormHelperText>
          </FormControl>
          ...
          <div className="two-inputs">
            ...
            >
              <FormLabel
                ...
                error={erro && !validateCpf(form.cpf)}
              >
                CPF*
              </FormLabel>
              <PatternFormat
                ...
                mask="_"
                error={(erro && !validateCpf(form.cpf)) || (erro && existsCpf)}
              ...
              <FormHelperText sx={{ m: 0, color: "red" }}>
                ...
                {erro && existsCpf && "CPF já cadastrado"}
              </FormHelperText>
            </FormControl>
            ...
            >
              ...
              <PatternFormat
                ...
                name="phone"
                onChange={(event) => handleChangeForm(event)}
                value={form.phone} //verificar como foi chamado o campo de telefone no backend
                size="small"
              />
              <FormHelperText sx={{ m: 0, color: "red" }}>
